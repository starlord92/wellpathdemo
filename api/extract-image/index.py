...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            if content_type and "application/json" in content_type and body:
                try:
                    ...
                    if url and url.startswith("http"):
                        ...
                        if not filename or filename == "upload.jpg":
            ...
            if not file_bytes:
                _send_headers(self, 400)
                self.wfile.write(json.dumps({"error": "Missing file. Send multipart with 'file' or 'image', or JSON {\"url\": \"https://...\"}.", "debug_body_len": len(body), "debug_content_type": content_type, "debug_body_prefix": body[:120].decode("latin-1") if body else ""}).encode("utf-8"))
                return
